...
class Board(tk.Frame):
    ...
    def button_click(self, buttonNumber):
        ...
        if self.isOver:
            ...
            # Buttons do nothing if they have already been taken
            self.info['text'] = 'That square is already taken.'
        else:
            # Code for a valid move
            self.update_game(buttonNumber, nextToken)
            # Get the computer's move after a human move (if in one player vs. computer mode)
    ...
    def update_game(self, buttonNumber, playerToken):
        """Updates the button text and if the game is over updates the label and the state of isOver"""
        # Set text colour and character for O and X
        button = self.buttons[buttonNumber]
        # Change the button's colour
        colours = {'X': '#ff0000', 'O': '#0000ff'}
        colour = colours[playerToken]
        # Need to change ativeforeground, otherwise text will be black when mouse is over the button
    ...
    def new_1player_game(self):
        """Sets up a new game with one humanb player against a computer oponent"""
    ...
    def new_2player_game(self):
        """Create a new game for two humans to play against each other"""
    ...
    def reset_game(self):
        """Resets various features in readiness for a new game"""
        # Set all buttons to be blank (empty string) 
        for button in self.buttons:
            button['text'] = ''
        # Set class variable so that game is in the active state
        self.isOver = False
...
class NamesWindow():
    """A class for the pop up that allows players' names to be entered"""
    ...
    def names_pop_up(self):
        """A pop up window that allows the names of the two players to be input"""
    ...
    def set_player_names(self):
        """Sets the players names when the OK button in the names pop up window is clicked"""
...
if __name__ == "__main__":
    ...
    # Don't set the geometry - grid will work out the size automatically
    #root.geometry('358x375')
    newgame = Board(root)
    root.mainloop()
